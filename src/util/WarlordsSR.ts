...
export async function getWarlordsSRFromName(username : string){
    if(!username.match("^[a-zA-Z0-9_]{1,16}$")){
        throw Error(`The username ${username} is in the wrong format!`)
    }
    const result = await axios.get<IWarlordsSrResponse>("https://warlordssr.unaussprechlich.net/api/" + username)
    if(result.status !== 200 || !result.data.success){
        throw Error(`Something went wrong while calling https://warlordssr.unaussprechlich.net/api/${username} !`)
    }
    return result.data.data
}

export function getPreviousNames(history : Array<MinecraftAPI.NameHistoryResponseModel>) {
    let names = ""
    for (const h of history.reverse()) {
        names += h.name + ", "
    }
    return names.substring(names.indexOf(', ') + 2, names.length - 2)
}


interface IWarlordsSrResponse{
    success : boolean
    data : {
        playername : string
        uuid : string
        warlords_sr : IWarlordsSR
        warlords_hypixel : IWarlordsHypixelAPI
        ranking : Ranking
        name_history : Array<MinecraftAPI.NameHistoryResponseModel>
        isInactive : boolean
    }
}
